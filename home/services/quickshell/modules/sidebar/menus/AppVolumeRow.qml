...
Item {
    ...
    readonly property string iconSource: {
        ...
        if (hint && hint.length > 0) {
            const guess = WorkspaceIcons.guessIcon(hint);
            if (WorkspaceIcons.iconExists(guess)) return Quickshell.iconPath(guess, "image-missing");
        }
        const fromName = WorkspaceIcons.guessIcon(node.properties["node.name"] ?? "");
